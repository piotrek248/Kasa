...
void Kasa::stan_kasy(){
    if(!zamkniety) {
        float stan_rzeczywisty =
                (banknot_200 * 200) + (banknot_100 * 100) + (banknot_50 * 50) + (banknot_20 * 20) + (banknot_10 * 10) +
                (moneta_5 * 5) + (moneta_2 * 2) + (moneta_1 * 1) + (moneta_05 * 0.5) +
                (moneta_02 * 0.2) + (moneta_01 * 0.1) + (moneta_005 * 0.05) + (moneta_002 * 0.02) + (moneta_001 * 0.01);
        int stan_calkowity = (int) stan_rzeczywisty;
        cout << "Stan kasy wynosi: " << (int) stan_rzeczywisty << " złotych "
             << ((stan_rzeczywisty - (float) stan_calkowity) * 100) << " groszy.";
    }else cout << "Kasa jest zamknięta przed sprawdzeniem stanu kasy otwórz kasę. " << endl;
}

void Kasa::ile_banknotow(){
    if (!zamkniety) {
        cout << "W kasie znajduje się: " << endl;
        cout << "- banknoty: " << banknot_200 << " po 200 zł, " << banknot_100 << " po 100zł. " << banknot_50
             << " po 50 zł, " << banknot_20 << " po 20 zł, " << banknot_10 << " po 10 zł, " << endl;
        cout << "-  monety zł: " << moneta_5 << " po 5 zł, " << moneta_2 << " po 2 zł, " << moneta_1 << " po 1 zł, "
             << endl;
        cout << "- monety gr: " << moneta_05 << " po 50 gr, " << moneta_02 << " po 20 gr, " << moneta_01
             << " po 10 gr, " << moneta_005 << " po 5 gr, " << moneta_002 << " po 2 gr, " << moneta_001 << " po 1 gr."
             << endl;
    } else cout << "Kasa jest zamknięta przed sprawdzeniem stanu kasy otwórz kasę. " << endl;
}
